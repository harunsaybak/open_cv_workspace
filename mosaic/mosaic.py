...
class Stitch:
    ...
    def process(self, ratio=0.75, reprojThresh=4.0, showMatches=False, outScale=1.0, inScale=1.0):
        ...
        base[windowShift[1]:self.images[0].shape[0] + windowShift[1],
        windowShift[0]:self.images[0].shape[1] + windowShift[0]] = self.images[0]  # todo, handle arbitrary base image
        ...
        for i, img in enumerate(self.images[:-1]):
            ...
            self.logger.info(
                "Extracting SIFT features for container, iteration {} of {}...".format(i + 1, len(self.images) - 1))
            ...
            kpsMatches = self.matchKeypoints(kps[i + 1], containerKpts, features[i + 1], containerFeats, ratio,
                                             reprojThresh)
            if kpsMatches == None:
    ...
    def addImage(self, image, container, first=False, transparent=False):
        ...
        mask = cv2.dilate(mask, kernel,
                          iterations=1)  # make the mask slightly larger so we don't get blank lines on the edges
...
if __name__ == "__main__":
    ...
    mosaic = Stitch(imgs)
    mosaic.process(ratio=0.75, reprojThresh=4.0, showMatches=args['showmatches'], outScale=args['outscale'],
